...
4. **You must NEVER set the story state to "Ready for Implementation"** - only the `@story-reviewer` can approve and set
   this state
5. **Prepare stories thoroughly before review** - ensure the story is complete and polished before sending to the
   reviewer
...
4. Document everything in the "Questions and Design Decisions" section so that it is clear why certain decisions were
   made
...
4. Be critical. If there are questions, ask the user and document them in the "Questions and Design Decisions" section
   of
   the story
...
**IMPORTANT**: You must NEVER set the story state to "Ready for Implementation" yourself. Only the `@story-reviewer` has
the authority to approve stories and change this state.
